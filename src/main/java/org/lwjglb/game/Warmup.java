...
public class Warmup implements IAppLogic {
    ...
    public BenchmarkInfo runWarmup() {
        ...
        for (int i = 0; i < totalRuns; i++) {
            Engine gameEng = new Engine("GPU-Benchmark: Warm-Up", new Window.WindowOptions(), main, i + 1);
            gameEng.start();
            ...
            if ((i + 1) % batchSize == 0) {
                ...
                System.out.println("\nAverage FPS after warm-up: " + averageFps);
            }
        }

        double finalScore = calculateFinalScore(allFpsValues);
        System.out.println("\nWarm-Up final score: " + finalScore);
